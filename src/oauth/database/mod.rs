...
use crate::{domain::ClientId, orm};

use self::resource::client::ClientName;
...
#[derive(Clone)]
pub struct Database {
    pub(crate) inner: DatabaseConnection,
}
...
impl Database {
    ...
    pub async fn get_client_name(&self, client_id: ClientId) -> Result<ClientName, StoreError> {
        let model = orm::get_client_app_by_client_id(client_id, &self.inner)
            .await
            .map_err(|_| StoreError::InternalError)?;

        Ok(ClientName {
            inner: model.name.unwrap_or(String::from("(Unknown App)")),
        })
    }
...
impl UserRecord {
    pub fn new(id: UserId, user: &str, given_name: &str) -> UserRecord {
        Self {
            id,
